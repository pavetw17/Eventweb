    ...
    protected void xuly(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (tbl_qt != null) {
			...
			if (request.getParameter("action").equalsIgnoreCase("getlength")) {
				int tongso_dong = bean.Publications_layTongSoDong_coDieuKien(ten_bai, String.valueOf(tu_ngay), String.valueOf(den_ngay));
				...
				ArrayList<tbl_tt_publications> list = bean.timkiem(ten_bai,String.valueOf(tu_ngay),String.valueOf(den_ngay),String.valueOf(limit),String.valueOf(offset));
				...
				str.append(" <script>	 "
						...
						+ " $.post('/eventweb/Publications_XoaTin', { "
						+ "	id_news: goTo,"
    	 				+ "	}, function(j){	 "
    	 				+ "		if(j=='success'){ "
				...
				for (int i = 0; i < list.size(); i++){
					...
					cnt ++;
		      str.append("	<td>" +  list.get(i).getName_publication() + "</td> ");
		      ...
		      str.append("  <a href='RedirectPage?action=xemnoidungPublications&id_tin_tuc=" + list.get(i).getId_publication() + "' target='_blank'");
		      str.append("	style='color: Red;'> View </a></td>");
		      str.append("	<td style='width: 30px'> ");
		      str.append("  <a href='Publications_SuaTin?id_tin_tuc=" + list.get(i).getId_publication()  + "'" );
		      str.append("	style='color: Red;'> Edit </a></td>");
		      str.append("	<td style='width: 30px'> ");
		      str.append("  <a href='" + list.get(i).getId_publication() + "' class='deleteButton'");
