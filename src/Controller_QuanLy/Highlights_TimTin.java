...
@WebServlet("/Highlights_TimTin")
public class Highlights_TimTin extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	@Resource(name = "EventDB")
	private DataSource ds;
	
    public Highlights_TimTin() {
        super();
    }
    ...
    protected void xuly(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (tbl_qt != null) {
			...
			if (request.getParameter("action").equalsIgnoreCase("getlength")) {
				int tongso_dong = bean.layTongSoDongHighlights_coDieuKien(ten_bai, String.valueOf(tu_ngay), String.valueOf(den_ngay));
				...
				ArrayList<tbl_news> list = bean.timkiem(ten_bai,String.valueOf(tu_ngay),String.valueOf(den_ngay),String.valueOf(limit),String.valueOf(offset));
				...
				str.append(" <script>	 "
						...
						+ " $.post('/eventweb/Highlights_XoaTin', { "
						+ "	id_news: goTo,"
    	 				+ "	}, function(j){	 "
    	 				+ "		if(j=='success'){ "
				...
				for (int i = 0; i < list.size(); i++){
					...
					cnt ++;
		      str.append("	<td>" +  list.get(i).getName_news() + "</td> ");
		      str.append("	<td>" +  list.get(i).getSummary() + " </td> ");
		      str.append("  <td style='font-size: small;'>" + func.DoiSoNguyenRaNgayThang(list.get(i).getStart_date()) + "</td>");
		      str.append("	<td style='width: 30px'> ");
		      str.append("  <a href='RedirectPage?action=xemnoidungHighlights&id_tin_tuc=" + list.get(i).getId_news() + "' target='_blank'");
		      str.append("	style='color: Red;'> View </a></td>");
		      str.append("	<td style='width: 30px'> ");
		      str.append("  <a href='Highlights_SuaTin?id_tin_tuc=" + list.get(i).getId_news()  + "'" );
		      str.append("	style='color: Red;'> Edit </a></td>");
		      str.append("	<td style='width: 30px'> ");
		      str.append("  <a href='" + list.get(i).getId_news() + "' class='deleteButton'");
